        ...
        <h1 class="text-4xl font-bold">
          ...
          <span
            class="
              ...
              text-green-800
            "
            ><span class="material-symbols-outlined align-middle mr-1 text-slate-700"> key </span
            >{{ idSup }}</span
          >
        </h1>
        <hr class="my-6 border border-emerald-600" />
        <form autocomplete="off" @submit.prevent="editSuplier">
          <!-- โซนอัปโหลดรูป -->
...
export default {
  ...
  setup(props, context) {
    ...
    // ID
    const idSup = ref(props.suplierProp.id);
    // ตัวแปรเก็บค่าใน Form
    const nameSupplier = ref(props.suplierProp.nameSupplier);
    const address = ref(props.suplierProp.address);
    const amphoe = ref(props.suplierProp.subDistrict);
    const phoneNum = ref(props.suplierProp.phoneNum);
    const province = ref(props.suplierProp.province);
    const district = ref(props.suplierProp.district);
    const zipcode = ref(props.suplierProp.zipcode);

    //สร้างอีก copy ของ prop เพื่อทำ reset เป็นค่า เดิม
    ...
    // imgSrc
    const imgSrc = ref(props.suplierProp.img);
    // เมื่อ img เป็นค่าว่าง
    if (imgSrc === "") {
    ...
    const resetForm = () => {
      ...
      if (imgSrc.value !== props.suplierProp.img) {
        // console.log('change')
        imgSrc.value = props.suplierProp.img;
      }
      
      
      nameSupplier.value = props.suplierProp.nameSupplier;
      address.value = props.suplierProp.address;
      amphoe.value = props.suplierProp.subDistrict;
      phoneNum.value = props.suplierProp.phoneNum;
      province.value = props.suplierProp.province;
      district.value = props.suplierProp.district;
      zipcode.value = props.suplierProp.zipcode;

    };

    // editSuplier
    const editSuplier = async () => {
      // เปิดโหลด
      isLoad.value = true;
      // รับค่าที่ได้จาก component และ map ลงใน obj
      //TODO ระวังสับสนอำเภอ
      //TODO แก้ img ด้วยให้เอามาจาก props
      const suplier = {
        address: address.value,
        district: amphoe.value,
        img: props.suplierProp.img,
      ...
      if (refSelect.value) {
        ...
        const url = await upload("supImg", idSup.value);
        suplier.img = url;
        
      }

      // Update ค่า img ใน FireStore
        await projectUpdate("Supplier", idSup.value, suplier);

      // เพิ่ม ID
      suplier.id = idSup.value;
