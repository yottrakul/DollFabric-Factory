<template>
  <div class="overlay">
    ...
    <div @click.self="closeOverlay" class="grid place-items-center h-screen">
      <!-- Container -->
      <div class="container w-full bg-slate-100 rounded-md grid grid-cols-4 h-[90%] max-h-[90%] overflow-hidden">
        <div class="zone1 col-span-3 overflow-y-auto scollHide mr-2">
        <!-- กล่องค้นหาและโซนตุ๊กตา -->
        <div class="block pl-5 -mb-2.5 pt-8"><span class="text-6xl font-semibold">ค้นหา</span><button @click="checkAvailable('DiOhaBeei5F4sa0ZoM6w', 30)">Test</button></div>
          <div class="px-4 py-4 sticky top-0 z-10 grid grid-cols-5 gap-x-5 bg-white/30 backdrop-blur">
            <!-- กล่องค้นหา -->
            <SearchBox class="col-span-4"/>
            <QuantitySearch/>
          </div>
            <div class="grid grid-cols-1 gap-y-10 gap-x-6 sm:grid-cols-2 lg:grid-cols-4 xl:gap-x-8 px-5">
            <!-- โซนตุ๊กตา -->
            <!-- <div v-for="product in products" :key="product.id" class="group relative"> -->
            <DollCard :amount="amount" />
          </div>
          
        </div>
        <div class="zone2 gap-y-4 grid h-full grid-cols-1 grid-rows-6 bg-green-600 outline-dashed outline-2 outline-offset-4 p-4 overflow-y-auto">
          <!-- ตะกร้าสินค้า -->
          <div class="header-cart text-center pt-20 row-span-1 sm:pt-5 lg:pt-10 xl:pt-20" >
          ...
          <div class="btn-cart grid grid-rows-2 row-span-1">
            <button class="bg-black transition-colors duration-200 hover:bg-white hover:text-black text-white w-full rounded-t-xl" >ตัด Stock</button>
            <button class="bg-red-500 transition-colors duration-200 hover:bg-white hover:text-red-500 text-white w-full rounded-b-xl" >ยกเลิก</button>
          </div>
        </div>
      </div>
    </div>
...
import QuantitySearch from '../components/QuantitySearch.vue'
import { ref } from '@vue/reactivity'
import getDolls from '../composibles/getDollDetails'
export default {
  components: {
    SearchBox, Cart, DollCard, QuantitySearch
  },
  props: ['stocks', 'dolls'],
  setup(props, context) {
    ...
    return { closeOverlay, amount, checkAvailable }
  }
}
</script>
